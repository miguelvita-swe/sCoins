...
import br.com.skyy.coins.api.event.CoinsChangeEvent;
import br.com.skyy.coins.api.event.CoinsTransferEvent;
...
public class SCoinsAPIImpl implements SCoinsAPI {
    ...
    public TransferResult transfer(UUID from, UUID to, long amount) {
        ...
        if (status == CoinsManager.TransferStatus.SUCCESS)          return TransferResult.SUCCESS;
        if (status == CoinsManager.TransferStatus.NOT_ENOUGH_COINS) return TransferResult.NOT_ENOUGH_COINS;
        if (status == CoinsManager.TransferStatus.TARGET_MAX)       return TransferResult.RECEIVER_MAX_REACHED;
