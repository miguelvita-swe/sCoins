...
import br.com.skyy.coins.storage.DatabaseManager;
import br.com.skyy.coins.Main;
import br.com.skyy.coins.manager.CheckManager;
import br.com.skyy.coins.manager.CoinsManager;
import br.com.skyy.coins.manager.CooldownManager;
...
import br.com.skyy.coins.util.MenuConfig;
...
public class CoinsCommand implements CommandExecutor {
    ...
    public CoinsCommand(CoinsManager coinsManager, FileStorage fileStorage, Messages messages,
                        ...
                        Main plugin, int minTransfer, NpcManager npcManager,
                        MenuConfig menuConfig, CheckManager checkManager) {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        ...
        // /coins — menu ou saldo dependendo de general.money-menu
        if (args.length == 0) {
            if (!(sender instanceof Player)) {
                // Console: sempre mostra saldo (não tem UUID, sem sentido abrir menu)
                messages.send(sender, "only-players");
                return true;
            }
            Player player = (Player) sender;
            boolean moneyMenu = config.getBoolean("general.money-menu", true);
            if (moneyMenu) {
                MainMenu.open(player, coinsManager, transactionManager, toggleManager, fileStorage, menuConfig.get());
            } else {
                messages.send(player, "balance-self",
                        "{coins}", br.com.skyy.coins.util.CoinsFormatter.format(coinsManager.getCoins(player.getUniqueId())));
            }
            return true;
        }
        ...
        // /coins top — menu ou chat dependendo de general.top-menu
        if (args[0].equalsIgnoreCase("top") && args.length == 1) {
            ...
            boolean topMenu = config.getBoolean("general.top-menu", true);
            if (topMenu) {
                if (!(sender instanceof Player)) { messages.send(sender, "only-players"); return true; }
                Player player = (Player) sender;
        ...
        if (args[0].equalsIgnoreCase("cheque")) {
            if (!(sender instanceof Player)) { messages.send(sender, "only-players"); return true; }
            if (!sender.hasPermission("scoins.use.cheque")) { messages.send(sender, "no-permission"); return true; }
            if (args.length != 2) { messages.send(sender, "cheque-invalid-value"); return true; }
            Player player = (Player) sender;
            long amount = parseAmount(sender, args[1]);
            if (amount < 0) return true;
            if (amount < minTransfer) {
                messages.send(sender, "cheque-min", "{min}", CoinsFormatter.format(minTransfer));
                SoundUtil.play(player, "error");
                return true;
            }
            if (!checkManager.issueMainCheck(player, amount)) {
                messages.send(sender, "cheque-no-balance");
                SoundUtil.play(player, "error");
                return true;
            }
            messages.send(player, "cheque-success", "{coins}", CoinsFormatter.format(amount));
            return true;
        }
        ...
        if (args[0].equalsIgnoreCase("db") && args.length == 2 && args[1].equalsIgnoreCase("status")) {
            ...
            DatabaseManager db = fileStorage.getDatabaseManager();
        ...
        if (args.length == 1) {
            ...
            // Jogador offline — verifica se transações offline estão habilitadas
            if (!config.getBoolean("general.offline-transactions", true)) {
                messages.send(sender, "transfer-offline-disabled");
                return true;
            }

            // Consulta assíncrona para não bloquear a main thread.
    ...
    /**
     * Exibe o ranking de coins no chat quando general.top-menu: false.
     * Suporta {magnata}, {prefixo} (via PlaceholderAPI), {tags} e {delay}.
     */
    private void sendTopChat(CommandSender sender) {
        List<String[]> top = fileStorage.getTopPlayers(10);
        int delay = config.getInt("general.top-delay", 600);

        String richFormatTemplate   = config.getString("top-chat.rich-format", "{magnata} ");
        String prefixFormatTemplate = config.getString("top-chat.prefix-format", "%luckperms_prefix% ");
        String tagsTemplate         = config.getString("top-chat.tags", "{magnata}{prefixo}");
        String playerFormat         = config.getString("top-chat.player-format",
                "  &f{pos}º {tags}&7{player} &8→ &2⛃&a{coins} &ecoins");
        String messageTemplate      = config.getString("top-chat.message",
                "\n &6&lTOP COINS &7(Atualizado a cada {delay}s)\n\n{formato}\n ");

        String[] magnataData = fileStorage.getMagnata();
        String magnataName   = magnataData != null ? magnataData[0] : null;
        String topTag        = br.com.skyy.coins.util.TextUtil.color(
                config.getString("general.top-tag", "&6&l[$]"));
        ...
        for (int i = 0; i < top.size(); i++) {
            ...
            String coinsFormatted = CoinsFormatter.format(Long.parseLong(top.get(i)[1]));
            int    pos            = i + 1;

            // {magnata} — vazio se não for o magnata, remove automaticamente se só espaços
            String magnataTag = "";
            if (playerName.equals(magnataName)) {
                String raw = richFormatTemplate.replace("{magnata}", topTag);
                if (!raw.trim().isEmpty()) magnataTag = br.com.skyy.coins.util.TextUtil.color(raw);
            }

            // {prefixo} — via PlaceholderAPI se disponível, senão vazio
            String prefixTag = "";
            if (hasPapi) {
                org.bukkit.OfflinePlayer op = plugin.getServer().getOfflinePlayer(playerName);
                String raw = me.clip.placeholderapi.PlaceholderAPI
                        .setPlaceholders(op, prefixFormatTemplate);
                if (!raw.trim().isEmpty()) prefixTag = br.com.skyy.coins.util.TextUtil.color(raw);
            }

            // {tags} — combina magnata + prefixo conforme config
            String tags = br.com.skyy.coins.util.TextUtil.color(
                    tagsTemplate
                            .replace("{magnata}", magnataTag)
                            .replace("{prefixo}", prefixTag));
            ...
            formato.append(br.com.skyy.coins.util.TextUtil.color(line));
            if (i < top.size() - 1) formato.append("\n");
        }

        if (top.isEmpty()) {
            formato.append(br.com.skyy.coins.util.TextUtil.color(
                    "  &7Nenhum jogador no ranking ainda."));
        }

        String finalMsg = messageTemplate
                .replace("{formato}", formato.toString())
                .replace("{delay}",   String.valueOf(delay));

        for (String line : br.com.skyy.coins.util.TextUtil.color(finalMsg).split("\n", -1)) {
